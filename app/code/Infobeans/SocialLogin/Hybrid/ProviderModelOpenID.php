...
class ProviderModelOpenID extends ProviderModel {
	...
	function initialize() {
		...
		require_once Auth::$config["path_libraries"] . "OpenID/LightOpenID.php";
		...
		$hostPort = parse_url(Auth::$config["base_url"], PHP_URL_PORT);
		$hostUrl = parse_url(Auth::$config["base_url"], PHP_URL_HOST);
		...
		$this->api = new LightOpenID($hostUrl, Auth::$config["proxy"]);
	}
