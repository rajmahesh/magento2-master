...
namespace Infobeans\SocialLogin\Hybrid;
...
class LightOpenID
{
    ...
    function __construct($host, $proxy = null)
    {
        ...
        if(!function_exists('curl_init') && !in_array('https', stream_get_wrappers())) {
            throw new Exception('You must have either https wrappers or curl enabled.');
        }
    }
        ...
        if (!empty($proxy)) {
            ...
            if ($proxy && !empty($proxy['host'])) {
                ...
                if (array_key_exists('port', $proxy)) {
                    ...
                    if ($proxy['port'] <= 0) {
                        throw new Exception('The specified proxy port number is invalid.');
                    }
                }
    ...
    protected function request_streams($url, $method='GET', $params=array(), $update_claimed_id)
    {
        if(!$this->hostExists($url)) {
            throw new Exception("Could not connect to $url.", 404);
        }
    ...
    {
        if (!$url) throw new Exception('No identity supplied.');
        ...
        for ($i = 0; $i < 5; $i ++) {
            ...
            throw new Exception("No OpenID Server found at $url", 404);
        }
        throw new Exception('Endless redirection!', 500);
    }
